...
class Slider(CanvasItem):
    """
    ## A Canvas Node to render a Slider with an adjustable knob.
    ...
    def draw(self):
        if self.visible:
            img, knx      = self._draw_onto_screen(self.scale_x * (self.properties["value"] / abs(self.properties["maximum"] - self.properties["minimum"])))
    ...
    def _draw_onto_screen(self, width):
        ...
        knob     = engine.thm.draw_thing("knob", [self.runtime_data["rendererpos"][0]+width, self.runtime_data["rendererpos"][1]], [self.scale_y/13,self.scale_y/13], self.window_id, self.anchor, self.layer, batch=self.barfbatch)
